...
<head>
    ...
    <link rel="stylesheet"
          href="${pageContext.request.contextPath}/static/css/hospital/doctorSelect.css">
</head>
<body class="screen">
...
<div class="doctor-notices card">
    <!-- 1) 목록 전체에 공지 있는 의사가 하나라도 있는지 플래그 계산 -->
    <c:set var="hasAnyNotice" value="false"/>
    <c:forEach var="d" items="${doctorList}">
        <c:if test="${not empty d.notices}">
            <c:set var="hasAnyNotice" value="true"/>
        </c:if>
    </c:forEach>

    <!-- 2) 플래그로 분기: 하나라도 있으면 공지들만, 없으면 '없습니다' 한 줄 -->
    <c:choose>
        <c:when test="${hasAnyNotice}">
                    ...
                    <table class="card-schedule">
                        ...
                        <tr>
                            ...
                            <c:forEach var="day" items="${doctor.schedule}">
                                <td><span
                                        class="${day.amFlag ? 'dot' : 'dash'}">${day.amFlag ? '●' : '–'}</span>
                                </td>
                            </c:forEach>
                        </tr>
                        <tr>
                            ...
                            <c:forEach var="day" items="${doctor.schedule}">
                                <td><span
                                        class="${day.pmFlag ? 'dot' : 'dash'}">${day.pmFlag ? '●' : '–'}</span>
                                </td>
                            </c:forEach>
                        </tr>
                        </tbody>
