...
public class HomeDAO {
  ...
  private static final String SQL_TODAY_FIRST_RESERVATION_BY_UUID = """
        SELECT r.appointment_at,
               dpt.name AS department_name,
               doc.name AS doctor_name
        FROM reservation r
  ...
  private static final String SQL_MY_LATEST_WAITING_BY_UUID = """
        SELECT wt.queue_no
        FROM waiting_ticket wt
        JOIN reception rc ON wt.reception_id = rc.reception_id
        JOIN member m ON m.member_id = rc.member_id
        WHERE m.uuid = ?
          AND wt.status IN ('WAITING','CALLED','IN_SERVICE')
        ORDER BY wt.created_at DESC
        LIMIT 1
  ...
  public HomeTodaySummaryDTO getHomeSummaryByUuid(String uuid) {
    HomeTodaySummaryDTO dto = new HomeTodaySummaryDTO();

    try (Connection con = DBConnectionUtil.getConnection()) {

      // (1) 오늘 첫 예약 한 건 - uuid 기준
      try (PreparedStatement ps = con.prepareStatement(SQL_TODAY_FIRST_RESERVATION_BY_UUID)) {
        ps.setString(1, uuid);
        try (ResultSet rs = ps.executeQuery()) {
          if (rs.next()) {
            Timestamp apAt = rs.getTimestamp("appointment_at");
      ...
      // (2) 나의 최근 대기 순번 - uuid 기준
      try (PreparedStatement ps = con.prepareStatement(SQL_MY_LATEST_WAITING_BY_UUID)) {
        ps.setString(1, uuid);
        try (ResultSet rs = ps.executeQuery()) {
          if (rs.next()) {
            dto.setMyQueueNo(rs.getInt("queue_no"));
      ...
      throw new RuntimeException("홈 요약(uuid) 조회 실패", e);
    }
