...
public class ReceptionListService {
    ...
    // uuid 기반 조회 서비스 메서드
//  - 컨트롤러가 세션에서 uuid를 전달하면 DAO에서 uuid로 필터링하여 목록을 반환
    public List<ReceptionListItemDTO> getListByUuid(String uuid, String status, LocalDate from, LocalDate to) {
        return dao.findListByUuid(uuid, status, from, to);
    }
     ...
     */
    public CancelResult cancelReceptionTransactional(Long receptionId, String uuid, String reason) {
        try {
            return dao.cancelReceptionTransactional(receptionId, uuid, reason);
        } catch (Exception e) {
            e.printStackTrace();
